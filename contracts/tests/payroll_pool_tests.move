...
fun hire_from_treasury_suspend_resume_stop() {
    ...
    ts::next_tx(&mut sc, ORG);
    {
        ...
        assert!(tr::idle_value<SUI>(&t) >= 50 * UNIT - 1, 1); // ~50 left of original 100 after hire
        // After hire: started with 100, invested 40, ensure_idle divested (~100 idle), withdrew 50 → ~50 idle.
        assert!(tr::idle_value<SUI>(&t) == 50 * UNIT, 2);
        ts::return_shared(v);
        ts::return_shared(t);
    };
